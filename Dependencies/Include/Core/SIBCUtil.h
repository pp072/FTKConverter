...
#define SI_ULong	unsigned int		/*!< SI_ULong is defines as 'ULONG'. */
...
struct TinyVariant {
	...
	TinyVariant() { memset(this,0,sizeof(TinyVariant));};
	TinyVariant(SI_Int v,SI_Int t,SI_Int e )
	{ 
		variantType = v;
		tweakType = t;
		numElems = e;
...
public:
	...
	SIFilePtr& operator=( const SIFilePtr &other )
	{
		// check for equality (are we the same as what we're assigning to)
		if (this != &other) {
			fp = other.fp;
		}
		return *this;
...
class CSIBCString;
SI_Error FTK_GetURLParts(char* in_pURL, CSIBCString* out_pScheme, CSIBCString* out_pAuthority, CSIBCString* out_pPath, CSIBCString* out_pQuery, CSIBCString* out_pFragment);
void FTK_ConvertStringFromURL(CSIBCString &io_String);
void FTK_ConvertStringToURL(CSIBCString &io_String);
