...
public:
	...
	void SetRotation(float fX, float fY, float fZ)
	{
		...
		this->x[0] = (float)(cy * cz);
		this->x[1] = (float)(cy * sz);
		this->x[2] = (float)(-sy);

		this->x[4] = (float)(sx * sy * cz - cx * sz);
		this->x[5] = (float)(sx * sy * sz + cx * cz);
		this->x[6] = (float)(sx * cy);

		this->x[8] = (float)(cx * sy * cz + sx * sz);
		this->x[9] = (float)(cx * sy * sz - sx * cz);
		this->x[10] = (float)(cx * cy);
